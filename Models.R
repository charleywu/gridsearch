...
#Eric Schulz & Charley Wu September 2016
...
#LINEAR KERNEL
##############################################################################################################
#Linear Kernel is a special case that uses data-driven hyper-parameters) 
#model posteriors are read in from CSV files that were pre-computed using linearposterior.m

lingp<-function(participantsid, trialnumber, roundnumber, dataFolder = "linearKernel"){
  # CSV files are ordered first by participant, and then by round
  fileNum <- ((participantsid-1) * 8) + roundnumber
  postData<-read.csv(paste0(dataFolder,'/',fileNum,".csv"), skip=1,header=FALSE)[trialnumber,] #read in posterior data, index by trial number; skip the first row
  mu <- unlist(postData[1:121]) #expected rewards are the first 121
  sigma<- unlist(postData[122:242])
  predictions <-data.frame(mu=mu, sig=sigma)
  colnames(predictions) <- c("mu", "sig")
  return(predictions)
}
class(lingp) <- c(class(lingp), "linGP")
...
inertia<-function(participantsid, trialnumber, roundnumber, dataFolder = "blockdistance"){
  # CSV files are ordered first by participant, and then by round
  fileNum <- ((participantsid-1) * 8) + roundnumber
  postData<-read.csv(paste0(dataFolder,'/block',fileNum,".csv"), header=FALSE)[,trialnumber] #read in posterior data, index by trial number; skip the first row
  mu <- unlist(postData[1:121]) #expected rewards are the first 121
  sigma<- rep(0,121)
  predictions <-data.frame(mu=mu, sig=sigma)
  colnames(predictions) <- c("mu", "sig")
...
ucb<-function(out, pars, refactor=F){
  if (refactor==TRUE){
    ...
    outtotal<-(gamma*out$mu)+(beta_star*out$sig) #refactored parameters in combination with softmax tau, where gamma = 1/tau and beta_star = beta/tau
    #avoid borderline cases
    outtotal[outtotal<0]<-0.01
...
pmu<-function(out){
  ...
  sigs<-matrix(out$sig, nrow=nrow(out)/121, byrow=TRUE)
  outtotal<-mus
  #loop through
  for (i in 1:nrow(mus)){
...
poi<-function(out, y.star){
  ...
  pi<-pnorm((out$mu-y.star+laplace)/(out$sig+laplace)) 
...
exofimp<-function(out, y.star){
  ...
  laplace <- sqrt(.Machine$double.eps) #DEBUG SOLUTION TO NAN VALUES; LAPLACIAN SMOOTHING
  z<-(out$mu-y.star+laplace)/(out$sig+laplace)
  ei <-(out$mu-y.star)*pnorm(z)+out$sig*dnorm(z)
...
greedyVar <- function(out){
  outtotal<- out$sig #the value of each choice is solely based on the expected uncertainty
  #avoid borderline cases
  outtotal[outtotal<0]<-0.01
  outtotal[outtotal>100]<-100
  outtotal<-matrix(outtotal, nrow=nrow(out)/121, byrow=TRUE)
