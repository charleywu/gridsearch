...
#load packages
packages <- c('dplyr', 'plyr', 'jsonlite')
...
dataImport <- function(normalize=TRUE){
  ...
  #dummy data frame
  data<-data.frame(id=numeric(), trial=numeric(), x=numeric(), y=numeric(), z=numeric(), zmax=numeric(), scenario=numeric(), horizon=numeric(), round=numeric(), env=numeric(), delta_x=numeric())
  ...
  for (i in 1:nrow(dat)){
    ...
    id<-rep(i, sum(len))
    #dummy frame
    dummy<-data.frame(id, trial, x, y, z, zmax, scenario, horizon, round, env)
