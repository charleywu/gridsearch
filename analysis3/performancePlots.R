...
#Can also plot model simulated data
...
dplota<-ddply(d,~trial+Horizon+scenario,summarise,mean=mean(z), se=se(z))
dplota$Measure<-"Average"
dplotb<-ddply(d,~trial+Horizon+scenario+trial,summarise,mean=mean(zmax), se=se(zmax))
dplotb$Measure<-"Maximum found"

#Final maximum value at the end of each round
dplotc <- ddply(d, ~id+horizon+scenario+round,summarise,rmax=max(zmax))
dplotcFinal <- ddply(dplotc, ~id+horizon+scenario, summarise, max=mean(rmax), se=se(rmax))
...
dplot5a<-ddply(d,~trial5+scenario+Horizon,summarise,mean=mean(z), se=se(z))
dplot5a$Measure <- "Avg. Reward"
dplot5b<-ddply(d,~scenario+trial5+Horizon,summarise,mean=mean(zmax), se=se(zmax))
...
dfinal <- fromJSON("environments/agridat.json", flatten=TRUE)
mu<-mx<-matrix(0, 40, 10000)
for(i in 1:10000){
  m<-as.numeric(unlist(dfinal[[sample(1:20,1)]]$y))*100
...
#save results
dplotRand<-data.frame(trial=rep(1:40, 4),
...
dplotRand$trial5<-round((dplotRand$trial+1)/5)*5
dplotRand$trial5<-ifelse(dplotRand$trial5<5,0,dplotRand$trial5)

#Join to existing dataframes
dplot5RandSum<- ddply(dplotRand,~scenario+trial5+Horizon+Measure,summarise,mean=mean(mean), se=se(mean))
dplot5 <- rbind(dplot5a, dplot5b, dplot5RandSum)
...
p1
ggsave(filename = "plots/avgReward.pdf", plot = p1, height =2.6, width = 3,  units = "in")
...
p2<-ggplot(subset(dplot5,Measure=="Max. Reward"), aes(x=trial5, y=mean, colour=interaction(Horizon,scenario))) +
  ...
  theme(legend.position="none", strip.background=element_blank(), legend.key=element_rect(color=NA))
p2
ggsave(filename = "plots/maxReward.pdf", plot = p2,  height =2.6, width = 3,  units = "in")
...
p2a
ggsave(filename = "plots/maxFinalReward.pdf", plot = p2a, height =2.2, width = 3, units = "in")
...
summarydf <- ddply(d, .(id, scenario, horizon, round), summarize, uniqueTiles = uniqueTiles(x,y)) 
...
uniqueRepeatDF <- melt(summarydf, id.vars=c("id", "scenario", "horizon","round"), measure.vars = c("uniqueTiles", "repeats"))
...
sampleSize <- 400000
randomDF <- data.frame(x=sample(x = seq(0:10), size = sampleSize, replace=TRUE), y=sample(x = seq(0:10), size = sampleSize, replace=TRUE), scenario = rep(c("Accumulators", "Maximizers"), sampleSize))
randomDF <- randomDF %>%
...
ggsave(filename = "plots/localityofSampling.pdf", plot = p4,height =2.5, width = 3, units = "in")
