...
#batchName <- 'recoveryLocalGP' #saves output csv files in 'modelResults/batchName/*.csv'
batchName <- 'recoveryLocalBMT' #saves output csv files in 'modelResults/batchName/*.csv'
#Simulated data file
simFile <- 'simDataBMTLocal.csv'
#simFile <- 'simDataGPLocal.csv'


#InertiaWeighting of acquisition functions used in Localized models
inertiaWeight <- TRUE
blockDistanceFolder <- "bmtLocalBlockDistance" #folder where pre-computed block distance of simulated data is kept
#blockDistanceFolder <- "gpLocalBlockDistance" #folder where pre-computed block distance of simulated data is kept
...
##############################################################################################################
#Compile Simulateds Data
##############################################################################################################
data<- read.csv(paste0('ExperimentData/',simFile), header=TRUE)
data$z<-(data$z-50)/100 #normalize data

if (inertiaWeight){blocks <- readBlockDistance(subjectId,dataFolder = blockDistanceFolder)} #if localized models, initialize inertia weights by reading pre-computed csv files
##############################################################################################################
#Model Fitting
##############################################################################################################
