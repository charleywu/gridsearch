#Model cross validation for simulated data (model paramterized with human estimates)
...
batchName = 'recoveryGPUCB' #saves output csv files in 'modelResults/batchName/*.csv'
#InertiaWeighting of acquisition functions used in Local-GP models
inertiaWeight <- FALSE

#Cluster id from qsub
#DEBUG 
#clusterid <- runif(1, 1, 81)
...
##############################################################################################################
data <- read.csv('ExperimentData/simDataGP.csv', header=TRUE)
data$y<-(data$y-50)/100 #normalize data
##############################################################################################################
#Model Fitting
##############################################################################################################
