...
grid <- seq(1,30)
pairwiseDistances <- sapply(grid, function(i) sapply(grid, function(j) abs(i - j)))
...
for (lambda in c(0.5, 1, 2)){
  ...
  sampledEnvs <- mvrnorm(n = reps, mu = rep(0,30), kernelFun(grid,grid,c(lambda,lambda,1,.0001))) 
  sampledEnvs <- (sampledEnvs- min(sampledEnvs)) / (max(sampledEnvs)- min(sampledEnvs)) * 100
